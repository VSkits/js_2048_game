...
function showMessage() {
    ...
    for(const key in messages)if (Object.hasOwnProperty.call(messages, key)) {
 ...
 */ class Game {
    ...
    moveLeft() {
        if (this.getStatus() !== Game.gameStatuses.playing) return;
    ...
    addCells(count = 1) {
        ...
        else if (!this.isStateValid(state)) this.status = Game.gameStatuses.lose;
    }
    resetState() {
        this.state = this.initialState.map((row)=>[
                ...row
